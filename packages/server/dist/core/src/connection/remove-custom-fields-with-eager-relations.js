...
function removeCustomFieldsWithEagerRelations(qb, relations = []) {
    ...
    if (customFieldsMetadata) {
        const customFieldRelationsWithEagerRelations = customFieldsMetadata.relations.filter(relation => {
            return (!!relation.inverseEntityMetadata.ownRelations.find(or => or.isEager === true) ||
                relation.inverseEntityMetadata.embeddeds.find(em => em.propertyName === 'customFields' && em.relations.find(emr => emr.isEager)));
        });
