...
let PromotionService = class PromotionService {
    constructor(connection, configService, channelService, listQueryBuilder, configArgService, customFieldRelationService, eventBus) {
    ...
    async validateCouponCode(ctx, couponCode, customerId) {
        ...
        if (!promotion ||
            ...
            !promotion.channels.find(c => utils_1.idsAreEqual(c.id, ctx.channelId))) {
    ...
    async runPromotionSideEffects(ctx, order, promotionsPre) {
        ...
        for (const activePre of promotionsPre) {
            if (!promotionsPost.find(p => utils_1.idsAreEqual(p.id, activePre.id))) {
        ...
        for (const activePost of promotionsPost) {
            if (!promotionsPre.find(p => utils_1.idsAreEqual(p.id, activePost.id))) {
