...
let ProductOptionGroupService = class ProductOptionGroupService {
    ...
    async deleteGroupAndOptionsFromProduct(ctx, id, productId) {
        ...
        if (0 < hasOptionsWhichAreInUse) {
            ...
            const product = await this.connection
                .getRepository(ctx, index_2.Product)
                .findOne(productId, { relations: ['optionGroups'] });
            if (product) {
                product.optionGroups = product.optionGroups.filter(og => !utils_1.idsAreEqual(og.id, id));
                await this.connection.getRepository(ctx, index_2.Product).save(product, { reload: false });
            }
            // TODO: V2 rely on onDelete: CASCADE rather than this manual loop
            for (const translation of optionGroup.translations) {
                await this.connection
                    .getRepository(ctx, product_option_group_translation_entity_1.ProductOptionGroupTranslation)
                    .remove(translation);
            }
            try {
                ...
                index_1.Logger.error(e.message, undefined, e.stack);
            }
        }
        this.eventBus.publish(new product_option_group_event_1.ProductOptionGroupEvent(ctx, optionGroup, 'deleted', id));
        return {
    ...
    async groupOptionsAreInUse(ctx, productOptionGroup) {
        return this.connection
            .getRepository(ctx, index_2.ProductVariant)
