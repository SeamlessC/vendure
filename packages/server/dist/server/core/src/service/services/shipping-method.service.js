...
let ShippingMethodService = class ShippingMethodService {
    constructor(connection, configService, listQueryBuilder, channelService, configArgService, translatableSaver, customFieldRelationService, eventBus, translator) {
    ...
    async findOne(ctx, shippingMethodId, includeDeleted = false, relations = []) {
        const shippingMethod = await this.connection.findOneInChannel(ctx, shipping_method_entity_1.ShippingMethod, shippingMethodId, ctx.channelId, Object.assign({ relations }, (includeDeleted === false ? { where: { deletedAt: null } } : {})));
