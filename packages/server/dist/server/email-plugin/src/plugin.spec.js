...
const testing_2 = require("@vendure/testing");
const fs_1 = require("fs");
const path_1 = __importDefault(require("path"));
const stream_1 = require("stream");
const default_email_handlers_1 = require("./default-email-handlers");
...
describe('EmailPlugin', () => {
    ...
    let onSend;
    let module;
    const testingLogger = new testing_2.TestingLogger(() => jest.fn());
    async function initPluginWithHandlers(handlers, options) {
        onSend = jest.fn();
        module = await testing_1.Test.createTestingModule({
            imports: [
                ...
                plugin_1.EmailPlugin.init(Object.assign({ templatePath: path_1.default.join(__dirname, '../test-templates'), transport: {
                        type: 'testing',
                        onSend,
                    }, handlers }, options)),
            ],
        ...
        core_1.Logger.useLogger(testingLogger);
        module.useLogger(new core_1.Logger());
        const plugin = module.get(plugin_1.EmailPlugin);
        eventBus = module.get(core_1.EventBus);
    ...
    it('setting from, recipient, subject', async () => {
        const ctx = core_1.RequestContext.deserialize({
            _channel: { code: shared_constants_1.DEFAULT_CHANNEL_CODE },
            _languageCode: core_1.LanguageCode.en,
        });
        const handler = new event_listener_1.EmailEventListener('test')
            .on(MockEvent)
    ...
    describe('event filtering', () => {
        ...
        it('multiple filters', async () => {
            ...
            await pause();
            expect(onSend).not.toHaveBeenCalled();
            const ctxWithUser = core_1.RequestContext.deserialize(Object.assign(Object.assign({}, ctx), { _session: { user: { id: 42 } } }));
    ...
    describe('templateVars', () => {
        ...
        it('interpolates body with property from entity', async () => {
            const handler = new event_listener_1.EmailEventListener('test')
                ...
                .setRecipient(() => 'test@test.com')
                .setSubject('Hello')
                .setTemplateVars(event => ({ order: new core_1.Order({ subTotal: 123 }) }));
            await initPluginWithHandlers([handler]);
            eventBus.publish(new MockEvent(ctx, true));
            await pause();
    ...
    describe('handlebars helpers', () => {
        ...
        it('formateDate', async () => {
            const handler = new event_listener_1.EmailEventListener('test-helpers')
                .on(MockEvent)
                .setFrom('"test from" <noreply@test.com>')
                .setRecipient(() => 'test@test.com')
                .setSubject('Hello')
    ...
    describe('multiple configs', () => {
        ...
        it('additional LanguageCode', async () => {
            const handler = new event_listener_1.EmailEventListener('test')
                ...
                .addTemplate({
                channelCode: 'default',
                languageCode: core_1.LanguageCode.de,
                templateFile: 'body.de.hbs',
                subject: 'Servus, {{ name }}!',
            });
            await initPluginWithHandlers([handler]);
            const ctxTa = core_1.RequestContext.deserialize(Object.assign(Object.assign({}, ctx), { _languageCode: core_1.LanguageCode.ta }));
            ...
            const ctxDe = core_1.RequestContext.deserialize(Object.assign(Object.assign({}, ctx), { _languageCode: core_1.LanguageCode.de }));
        ...
        it('set LanguageCode', async () => {
            const handler = new event_listener_1.EmailEventListener('test')
                ...
                .setSubject('Hello, {{ name }}!')
                .setRecipient(() => 'test@test.com')
                .setLanguageCode(() => core_1.LanguageCode.de)
                .setTemplateVars(() => ({ name: 'Test' }))
                .addTemplate({
                channelCode: 'default',
                languageCode: core_1.LanguageCode.de,
            ...
            const ctxEn = core_1.RequestContext.deserialize(Object.assign(Object.assign({}, ctx), { _languageCode: core_1.LanguageCode.en }));
    ...
    describe('loadData', () => {
        it('loads async data', async () => {
            ...
            await initPluginWithHandlers([handler]);
            eventBus.publish(new MockEvent(core_1.RequestContext.deserialize({
                _channel: { code: shared_constants_1.DEFAULT_CHANNEL_CODE },
                _languageCode: core_1.LanguageCode.en,
            }), true));
            await pause();
        ...
        it('only executes for filtered events', async () => {
            ...
            await initPluginWithHandlers([handler]);
            eventBus.publish(new MockEvent(core_1.RequestContext.empty(), false));
            eventBus.publish(new MockEvent(core_1.RequestContext.empty(), true));
            await pause();
    ...
    describe('attachments', () => {
        ...
        const TEST_IMAGE_PATH = path_1.default.join(__dirname, '../test-fixtures/test.jpg');
        ...
        it('attachment content as an image buffer', async () => {
            const imageFileBuffer = fs_1.readFileSync(TEST_IMAGE_PATH);
            const handler = new event_listener_1.EmailEventListener('test')
                .on(MockEvent)
                .setFrom('"test from" <noreply@test.com>')
                .setRecipient(() => 'test@test.com')
        ...
        it('attachment content as a string stream', async () => {
            const handler = new event_listener_1.EmailEventListener('test')
                ...
                .setAttachments(() => [
                {
                    content: stream_1.Readable.from(['hello']),
                },
            ]);
            await initPluginWithHandlers([handler]);
            eventBus.publish(new MockEvent(ctx, true));
        ...
        it('attachment content as an image stream', async () => {
            const imageFileBuffer = fs_1.readFileSync(TEST_IMAGE_PATH);
            const imageFileStream = fs_1.createReadStream(TEST_IMAGE_PATH);
            const handler = new event_listener_1.EmailEventListener('test')
                .on(MockEvent)
                .setFrom('"test from" <noreply@test.com>')
                .setRecipient(() => 'test@test.com')
        ...
        it('raises a warning for large content attachments', async () => {
            ...
            eventBus.publish(new MockEvent(ctx, true));
            await pause();
            expect(testingLogger.warnSpy.mock.calls[0][0]).toContain(`Email has a large 'content' attachment (64k). Consider using the 'path' instead for improved performance.`);
        });
    });
    describe('orderConfirmationHandler', () => {
        beforeEach(async () => {
            await initPluginWithHandlers([default_email_handlers_1.orderConfirmationHandler], {
                templatePath: path_1.default.join(__dirname, '../templates'),
            });
        });
        const ctx = core_1.RequestContext.deserialize({
            _channel: { code: shared_constants_1.DEFAULT_CHANNEL_CODE },
        ...
        it('filters events with wrong order state', async () => {
            eventBus.publish(new core_1.OrderStateTransitionEvent('AddingItems', 'ArrangingPayment', ctx, order));
            await pause();
            expect(onSend).not.toHaveBeenCalled();
            eventBus.publish(new core_1.OrderStateTransitionEvent('AddingItems', 'Cancelled', ctx, order));
            await pause();
            expect(onSend).not.toHaveBeenCalled();
            eventBus.publish(new core_1.OrderStateTransitionEvent('AddingItems', 'PaymentAuthorized', ctx, order));
            await pause();
            expect(onSend).not.toHaveBeenCalled();
            eventBus.publish(new core_1.OrderStateTransitionEvent('ArrangingPayment', 'PaymentSettled', ctx, order));
            await pause();
            expect(onSend).toHaveBeenCalledTimes(1);
        });
    ...
    describe('error handling', () => {
        it('Logs an error if the template file is not found', async () => {
            ...
            const handler = new event_listener_1.EmailEventListener('no-template')
                .on(MockEvent)
                .setFrom('"test from" <noreply@test.com>')
                .setRecipient(() => 'test@test.com')
                .setSubject('Hello {{ subjectVar }}');
        ...
        it('Logs a Handlebars error if the template is invalid', async () => {
            ...
            testingLogger.errorSpy.mockClear();
            const handler = new event_listener_1.EmailEventListener('bad-template')
                .on(MockEvent)
                .setFrom('"test from" <noreply@test.com>')
                .setRecipient(() => 'test@test.com')
                .setSubject('Hello {{ subjectVar }}');
...
class FakeCustomSender {
}
const pause = () => new Promise(resolve => setTimeout(resolve, 100));
class MockEvent extends core_1.VendureEvent {
    constructor(ctx, shouldSend) {
        super();
        this.ctx = ctx;
