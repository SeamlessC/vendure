...
let CollectionService = class CollectionService {
    constructor(connection, channelService, assetService, facetValueService, listQueryBuilder, translatableSaver, eventBus, jobQueueService, configService, slugValidator, configArgService, customFieldRelationService, translator, roleService) {
                ...
                for (const collectionId of job.data.collectionIds) {
                    ...
                    if (collection) {
                        ...
                        try {
                            ...
                            vendure_logger_1.Logger.error(`An error occurred when processing the filters for the collection "${translatedCollection.name}" (id: ${collection.id})`);
    ...
    async removeCollectionsFromChannel(ctx, input) {
        ...
        if (utils_1.idsAreEqual(input.channelId, defaultChannel.id)) {
            throw new errors_1.UserInputError('error.collections-cannot-be-removed-from-default-channel');
        }
