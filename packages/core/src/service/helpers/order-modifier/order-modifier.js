...
const entity_hydrator_service_1 = require("../entity-hydrator/entity-hydrator.service");
...
let OrderModifier = class OrderModifier {
    constructor(connection, configService, orderCalculator, paymentService, countryService, stockMovementService, productVariantService, customFieldRelationService, promotionService, eventBus, entityHydrator, historyService, translator) {
    ...
    async modifyOrder(ctx, input, order) {
        ...
        if (input.couponCodes) {
            ...
            const orderItems = order.lines.reduce((all, line) => all.concat(line.items), []);
            await this.connection.getRepository(ctx, order_item_entity_1.OrderItem).save(orderItems, { reload: false });
        }
        else {
    ...
    async getAdjustmentFromNewlyAppliedPromotions(ctx, order, promotionsPre) {
        const newPromotionDiscounts = order.discounts
            ...
            return !promotionsPre.find(p => utils_1.idsAreEqual(p.id, promotionId));
        })
            .filter(discount => {
        ...
        if (newPromotionDiscounts.length) {
            return -shared_utils_1.summate(newPromotionDiscounts, 'amountWithTax');
        }
        return 0;
    }
    ...
    async customFieldsAreEqual(ctx, orderLine, inputCustomFields, existingCustomFields) {
        ...
        for (const def of customFieldDefs) {
            ...
            if (def.type !== 'relation' && existingValue !== undefined) {
                ...
                const inputId = shared_utils_1.getGraphQlInputName(def);
