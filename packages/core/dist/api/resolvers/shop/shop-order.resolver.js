...
const shop_closed_guard_1 = require("../../common/shop-closed.guard");
...
let ShopOrderResolver = class ShopOrderResolver {
    ...
    async order(ctx, args, relations) {
        ...
        const order = await this.orderService.findOne(ctx, args.id, unique_1.unique([...relations, ...requiredRelations]));
    ...
    async activeOrder(ctx, relations) {
    ...
    async orderByCode(ctx, args, relations) {
        if (ctx.authorizedAsOwnerOnly) {
            const requiredRelations = ['customer', 'customer.user'];
            const order = await this.orderService.findOneByCode(ctx, args.code, unique_1.unique([...relations, ...requiredRelations]));
            if (order &&
...
__decorate([
    ...
    allow_decorator_1.Allow(generated_shop_types_1.Permission.Owner),
    __param(0, request_context_decorator_1.Ctx()),
    __param(1, relations_decorator_1.Relations(order_entity_1.Order)),
    __metadata("design:type", Function),
