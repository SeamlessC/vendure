...
let FacetService = class FacetService {
    ...
    findByCode(ctxOrFacetCode, facetCodeOrLang, lang) {
        ...
        return repository
            ...
            .then(facet => facet && translate_entity_1.translateDeep(facet, languageCode, ['values', ['values', 'facet']]));
    }
    /**
     * @description
    ...
    async assignFacetsToChannel(ctx, input) {
        ...
        await Promise.all([
            ...
            ...valuesToAssign.map(async (value) => this.channelService.assignToChannels(ctx, index_1.FacetValue, value.id, [input.channelId])),
        ]);
        return this.connection
            .findByIdsInChannel(ctx, facet_entity_1.Facet, facetsToAssign.map(f => f.id), ctx.channelId, {})
            .then(facets => facets.map(facet => translate_entity_1.translateDeep(facet, ctx.languageCode)));
    }
    /**
     * @description
    ...
    async removeFacetsFromChannel(ctx, input) {
        ...
        if (utils_1.idsAreEqual(input.channelId, defaultChannel.id)) {
            throw new common_2.UserInputError('error.facets-cannot-be-removed-from-default-channel');
        }
        ...
        for (const facet of facetsToRemove) {
            ...
            if (facet.values.length) {
                ...
                if (!isInUse || input.force) {
                    ...
                    await Promise.all(facet.values.map(fv => this.channelService.removeFromChannels(ctx, index_1.FacetValue, fv.id, [input.channelId])));
                    ...
                    results.push(new common_2.FacetInUseError(facet.code, productCount, variantCount));
                }
            }
        }
        return results;
