...
let ChannelService = class ChannelService {
     ...
     */
    async createCache() {
        return self_refreshing_cache_1.createSelfRefreshingCache({
    ...
    async assignToAllChannels(entity, ctx, defaultChannel) {
        ...
        const channelIds = unique_1.unique([...allIds, defaultChannel.id]);
        entity.channels = channelIds.map(id => ({ id }));
        this.eventBus.publish(new change_channel_event_1.ChangeChannelEvent(ctx, entity, [ctx.channelId], 'assigned'));
        return entity;
    }
    ...
    async ensureDefaultChannelExists() {
        ...
        const defaultChannel = await this.connection.rawConnection.getRepository(channel_entity_1.Channel).findOne({
            where: {
